BSDisplay - JavaFXアプリケーション

用途
- 企業の決算書に記載されている貸借対照表（BS）の内容を入力することで、グラフを出力することができるアプリケーションです。
...
- グラフ保存時に「保存フォルダ名」が入力されていない場合は「企業名-BS」が、「保存フォルダ名」も「企業名」も入力されていない場合は「SampleCorp-BS」がフォルダ名として出力されます。
